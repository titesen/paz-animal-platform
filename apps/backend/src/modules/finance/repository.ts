 ...
 */
export async function findDonationsByUser(
  userId: string,
): Promise<MonetaryDonation[]> {
  return await db
    .select()
 ...
 */
export async function findAllConfirmedDonations(
  limit: number = 50,
): Promise<MonetaryDonation[]> {
  return await db
    .select()
 ...
 */
export async function findInKindDonationById(
  donationId: string,
): Promise<InKindDonation | null> {
  const [donation] = await db
    .select()
