...
export async function findMediaById(mediaId: string) {
  const [result] = await db.select().from(media).where(eq(media.mediaId, mediaId)).limit(1);
...
export async function findMainMedia(entityType: string, entityId: string) {
  const [result] = await db
    ...
    .from(media)
    .where(
      and(eq(media.entityType, entityType), eq(media.entityId, entityId), eq(media.isMain, true)),
    )
    .limit(1);
 ...
 */
export async function setMainMedia(mediaId: string, entityType: string, entityId: string) {
...
export async function deleteMedia(mediaId: string) {
  const [result] = await db.delete(media).where(eq(media.mediaId, mediaId)).returning();
 ...
 */
export async function updateMedia(mediaId: string, data: { altText?: string; isMain?: boolean }) {
  const [result] = await db.update(media).set(data).where(eq(media.mediaId, mediaId)).returning();
