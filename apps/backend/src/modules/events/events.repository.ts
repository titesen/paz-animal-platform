...
export async function findEventById(
  ...
  const event = await db
    .select()
    .from(schema.events)
    .where(and(eq(schema.events.eventId, eventId), isNull(schema.events.deletedAt)))
    .limit(1);
 ...
 * Update event
 */
export async function updateEvent(eventId: string, data: Partial<Event>): Promise<Event | null> {
...
export async function createEventRegistration(data: {
  ...
  const [registration] = await db.insert(schema.eventRegistrations).values(data).returning();
 ...
 */
export async function findRegistrationsByEvent(eventId: string): Promise<EventRegistration[]> {
  return await db
    .select()
    .from(schema.eventRegistrations)
...
export async function createAttendance(data: {
  ...
  const [attendance] = await db.insert(schema.attendances).values(data).returning();
 ...
 */
export async function findAttendancesByEvent(eventId: string): Promise<Attendance[]> {
  return await db
    .select()
    .from(schema.attendances)
    .where(
      and(eq(schema.attendances.entityType, "EVENT"), eq(schema.attendances.entityId, eventId)),
    )
