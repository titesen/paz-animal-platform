...
2. **🧠 Hypothesize the Root Cause:**
   - Explain _why_ this is happening in the context of our architecture (e.g., "The Controller is trying to access a repository method that doesn't map the polymorphic relation correctly").
...
\*\*IA (Respuesta):\*\*
Como ya tiene el contexto, sabrá que probablemente es un problema de configuración de esquemas (\`search_path\`) en PostgreSQL o que te olvidaste de correr la migración, y te dará la solución exacta para Drizzle.
