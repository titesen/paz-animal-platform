...
Utilizamos Zod como un adaptador que toma las variables de entorno crudas (process.env que son strings inseguros) y las transforma en un objeto tipado y validado (env.PORT como número).

Justificación: Protege al sistema de fallar en runtime por variables faltantes o mal formadas.
...
El Service actúa como una fachada simple para el Controller. El Controller no sabe si para "Adoptar" hay que enviar emails, actualizar stocks o validar usuarios; solo llama a adoptionService.create().
...
Cada request pasa por una cadena de procesadores: Helmet (Seguridad) \-\> Cors \-\> JSON Parser \-\> Auth Middleware \-\> Controller. Si uno falla (o no llama a next()), la cadena se corta.
...
Justificación: Mantiene la UI sincronizada con el servidor sin necesidad de "prop drilling" o manejo manual de eventos.
