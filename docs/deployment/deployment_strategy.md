...
4. **Almacenamiento (Object Storage):**  
   ...
   * *Ventaja:* Sin costos de egreso (bandwidth).
...
| :---- | :---- | :---- | :---- | :---- |
| **Local** | feature/\* | localhost:5173 | Desarrollo diario. | Docker Local (paz\_animal\_db) |
...
* DATABASE\_URL (Connection String Postgres)  
* JWT\_SECRET (Firma de tokens)  
* R2\_ACCESS\_KEY\_ID & R2\_SECRET\_ACCESS\_KEY (Storage)  
* MERCADOPAGO\_ACCESS\_TOKEN (Pasarela de pagos)  
* NODE\_ENV (Debe ser production)
...
6. *Nota:* Si hubo una migración de base de datos destructiva, se requerirá intervención manual SQL. Por eso, **evitamos migraciones destructivas** (siempre ADD COLUMN, nunca DROP COLUMN inmediato).
